...
def test_bracket_nominal_matches_readme_section_3():
    # depth origin (75.0, -17.5, 35.0) mm; x_cam = +Y, y_cam = -X, z_cam = +Z
    assert _close(BRACKET_NOMINAL.translation, (0.075, -0.0175, 0.035))
    ...
    # a point 300 mm straight out of the lens sits 335 mm out of the flange face
    assert _close(HandEye().camera_to_flange((0, 0, 0.3)), (0.075, -0.0175, 0.335))
...
def test_extrinsics_move_the_colour_origin():
    ...
    assert _close(he.camera_to_flange((0, 0, 0.3)), (0.075, -0.0175 - 0.015, 0.335))
...
def test_locate_geometry_tool_down():
    ...
    # tool down => the camera looks along base -Z; the object is 0.335 below the flange
    assert _close(out["view_ray_base"], (0, 0, -1), 1e-9)
    assert out["point_base_m"][2] == pytest.approx(0.5 - 0.335, abs=1e-9)
    # approach = 50 mm short of the object along that ray, current tool orientation kept
    assert out["approach_pose"][2] == pytest.approx(0.5 - 0.335 + 0.05, abs=1e-9)
    ...
    assert out["point_base_m"][0] == pytest.approx(0.5 - 0.075, abs=1e-9)
